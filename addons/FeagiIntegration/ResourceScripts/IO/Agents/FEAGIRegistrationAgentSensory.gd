...
func _register_device() -> FEAGI_IOHandler_Base:
	if _data_retrieval_function.is_null():
		...
		return null
	var sensor_device: FEAGI_IOHandler_Base = FEAGIRunTime.mapping_config.sensor_register(_device_type_name, _device_name, _data_retrieval_function)
	return sensor_device

func _deregister_device() -> FEAGI_IOHandler_Base:
	var sensor_device: FEAGI_IOHandler_Base = FEAGIRunTime.mapping_config.sensor_deregister(_device_type_name, _device_name)
	return sensor_device
