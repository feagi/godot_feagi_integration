...
var registration_agent: FEAGIRegistrationAgentSensory
...
#NOTE: All FEAGIRegistrationAgent derived classes will attempt to autoregister upon the signal from [FEAGIRunTime]. This is desired
...
func setup_screenshot_system(FEAGI_camera_name: StringName) -> void:
	registration_agent = FEAGIRegistrationAgentSensory.new()
	registration_agent.set_current_device_name(FEAGI_camera_name)
	registration_agent.setup_device_type("camera", _capture_viewport)
	# From here, the registration agent automatically suscribes to the "signal_all_autoregister_sensors_to_register" signal from [FEAGIRunTime]
