...
var sequences: FEAGI_RunTime_SequencePlayerManager = null
...
func initialize_FEAGI_runtime(mapping_config: FEAGI_Genome_Mapping = null, endpoint_config: FEAGI_Resource_Endpoint = null) -> void:
	...
	# Add sequences (and autostart autostarting ones)
	sequences = FEAGI_RunTime_SequencePlayerManager.new()
	add_child(sequences)
	sequences.FEAGI_setup(mapping_config.predfined_input_sequences)
