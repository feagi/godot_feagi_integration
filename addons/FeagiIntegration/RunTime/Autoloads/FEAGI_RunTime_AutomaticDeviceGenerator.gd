...
func add_all_autogenerated_objects(FEAGI_sensors: Dictionary, FEAGI_motors: Dictionary) -> void:
	...
	for motor_name in FEAGI_motors:
		...
		if FEAGI_motors[motor_name] is FEAGI_IOHandler_Motor_Motor:
			if (FEAGI_motors[motor_name] as FEAGI_IOHandler_Motor_Motor).is_using_automatic_input_key_emulation():
				var input_emulators: FEAGI_Device_GodotInputEvents = FEAGI_Device_GodotInputEvents.new()
				add_child(input_emulators)
				input_emulators.setup_input_emulation((FEAGI_motors[motor_name] as FEAGI_IOHandler_Motor_Motor).device_name, (FEAGI_motors[motor_name] as FEAGI_IOHandler_Motor_Motor).automatically_emulate_keys)
				input_emulators.register_device()
