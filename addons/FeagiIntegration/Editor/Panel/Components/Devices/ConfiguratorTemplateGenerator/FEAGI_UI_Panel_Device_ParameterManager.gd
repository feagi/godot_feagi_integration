...
const FLOAT_PREFAB_PATH: StringName = "res://addons/FeagiIntegration/Editor/Panel/Components/Devices/ConfiguratorTemplateGenerator/Parameters/FEAGI_UI_Panel_Device_ParameterFloat.tscn"
const INT_PREFAB_PATH: StringName = "res://addons/FeagiIntegration/Editor/Panel/Components/Devices/ConfiguratorTemplateGenerator/Parameters/FEAGI_UI_Panel_Device_ParameterInt.tscn"
const LIST_PREFAB_PATH: StringName = "res://addons/FeagiIntegration/Editor/Panel/Components/Devices/ConfiguratorTemplateGenerator/Parameters/FEAGI_UI_Panel_Device_ParameterList.tscn"
...
func _generate_parameter_controls(object_parameters: Array[Dictionary], existing_values_for_device: Dictionary = {}) -> Array[FEAGI_UI_Panel_Device_ParameterBase]:
	var building_list: Array[FEAGI_UI_Panel_Device_ParameterBase] = []
	...
	for parameter: Dictionary in object_parameters:
		var object: FEAGI_UI_Panel_Device_ParameterBase = _spawn_parameter(parameter, existing_values_for_device)
		if object != null:
...
func _spawn_parameter(parameter_template: Dictionary, paremeter_given_values: Dictionary = {}) -> FEAGI_UI_Panel_Device_ParameterBase:
	# Validation
	...
	match(parameter_template["type"]):
		"string":
			...
			(appending as FEAGI_UI_Panel_Device_ParameterString).setup(label, description)
			if default_value:
				(appending as FEAGI_UI_Panel_Device_ParameterString).set_value(default_value)
		"boolean":
			appending = load(BOOL_PREFAB_PATH).instantiate()
			(appending as FEAGI_UI_Panel_Device_ParameterBool).setup(label, description)
			if default_value:
				(appending as FEAGI_UI_Panel_Device_ParameterBool).set_value(default_value)
		"integer":
			appending = load(INT_PREFAB_PATH).instantiate()
			(appending as FEAGI_UI_Panel_Device_ParameterInt).setup(label, description)
			if default_value:
				(appending as FEAGI_UI_Panel_Device_ParameterInt).set_value(default_value)
			if "min" in parameter_template:
				(appending as FEAGI_UI_Panel_Device_ParameterInt).set_min(parameter_template["min"])
			if "max" in parameter_template:
				(appending as FEAGI_UI_Panel_Device_ParameterInt).set_max(parameter_template["max"])
		"float":
			appending = load(FLOAT_PREFAB_PATH).instantiate()
			(appending as FEAGI_UI_Panel_Device_ParameterFloat).setup(label, description)
			if default_value:
				(appending as FEAGI_UI_Panel_Device_ParameterFloat).set_value(default_value)
			if "min" in parameter_template:
				(appending as FEAGI_UI_Panel_Device_ParameterFloat).set_min(parameter_template["min"])
			if "max" in parameter_template:
				(appending as FEAGI_UI_Panel_Device_ParameterFloat).set_max(parameter_template["max"])
		"percentage":
			appending = load(PERCENTAGE_PREFAB_PATH).instantiate()
			(appending as FEAGI_UI_Panel_Device_ParameterPercentage).setup(label, description)
			if default_value:
				(appending as FEAGI_UI_Panel_Device_ParameterPercentage).set_value(default_value)
			if "min" in parameter_template:
				(appending as FEAGI_UI_Panel_Device_ParameterPercentage).set_min(parameter_template["min"])
			if "max" in parameter_template:
				(appending as FEAGI_UI_Panel_Device_ParameterPercentage).set_max(parameter_template["max"])
		"object":
			...
			if paremeter_given_values.has(label):
				...
				(appending as FEAGI_UI_Panel_Device_ParameterObject).setup_internals(_generate_parameter_controls(subparameters_array))
		"vector2":
		...
		"vector3":
			...
			if default_value:
				(appending as FEAGI_UI_Panel_Device_ParameterVector3).set_value(default_value)
...
func _attempt_connect_visibity_toggle(toggling_object: FEAGI_UI_Panel_Device_ParameterBase, existing_objects: Array[FEAGI_UI_Panel_Device_ParameterBase]) -> void:
	if !toggling_object.flag_for_toggle_by_parameter_of_name.is_empty():
		for object in existing_objects:
			if object.name != toggling_object.flag_for_toggle_by_parameter_of_name:
				continue
			if object is FEAGI_UI_Panel_Device_ParameterBool:
				(object as FEAGI_UI_Panel_Device_ParameterBool).bool_changed.connect(toggling_object.set_visible)
				toggling_object.visible = (object as FEAGI_UI_Panel_Device_ParameterBool).get_value()
				return
	...
	if !toggling_object.flag_for_inverse_toggle_by_parameter_of_name.is_empty():
		for object in existing_objects:
			if object.name != toggling_object.flag_for_inverse_toggle_by_parameter_of_name:
				continue
			if object is FEAGI_UI_Panel_Device_ParameterBool:
				(object as FEAGI_UI_Panel_Device_ParameterBool).bool_changed_inversed.connect(toggling_object.set_visible)
				toggling_object.visible = !(object as FEAGI_UI_Panel_Device_ParameterBool).get_value()
				return
